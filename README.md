...
Production-ready HTTP client, server, router, and middleware toolkit for Alya
...
Add `http` to the `[dependencies]` section in your `alya.toml`:
...
| `new_config(name, count)` | `name = "World", count = 1` | `HttpConfig` | Constructs a new configuration struct. |
| `core_format_greeting(name)` | `name` | `string` | Core formatter producing `Hello, {name}!`. |
| `core_format_custom(config)` | `config: HttpConfig` | `string` | Formats greeting using prefix and name from config. |
